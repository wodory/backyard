...
CREATE POLICY "사용자는 자신의 정보만 업데이트할 수 있음" ON public.users FOR UPDATE USING (auth.uid() = id) WITH CHECK (auth.uid() = id);
...
CREATE POLICY "사용자는 자신의 프로필만 수정 가능" ON public.profiles FOR UPDATE USING (auth.uid() = id) WITH CHECK (auth.uid() = id);
...
CREATE POLICY "프로젝트 소유자만 프로젝트 생성/수정/삭제 가능" ON public.projects FOR ALL USING (auth.uid() = owner_id) WITH CHECK (auth.uid() = owner_id);
...
CREATE POLICY "프로젝트 소유자만 멤버 추가/삭제/수정 가능" ON public.project_members FOR ALL USING (EXISTS (SELECT 1 FROM public.projects p WHERE p.id = project_members.project_id AND p.owner_id = auth.uid())) WITH CHECK (EXISTS (SELECT 1 FROM public.projects p WHERE p.id = project_members.project_id AND p.owner_id = auth.uid()));
...
CREATE POLICY "카드 생성자만 카드 수정/삭제 가능" ON public.cards FOR UPDATE, DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "태그 생성자만 태그 수정/삭제 가능" ON public.tags FOR UPDATE, DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "엣지 생성자만 엣지 수정/삭제 가능" ON public.edges FOR UPDATE, DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "카드 소유자만 태그 연결/해제 가능" ON public.card_tags FOR INSERT, DELETE USING (EXISTS (SELECT 1 FROM public.cards c WHERE c.id = card_tags.card_id AND c.user_id = auth.uid()));
