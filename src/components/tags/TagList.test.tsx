...
describe('TagList', () => {
  ...
  it('태그 삭제 확인 시 API 호출이 이루어지고 태그가 목록에서 제거되어야 함', async () => {
    // Mock API 응답을 설정하여 태그가 성공적으로 삭제되도록 함
    (global.fetch as Mock).mockImplementation(async () => {
      // 비동기적으로 상태를 변경하기 위해 약간의 지연 추가
    ...
    // 다시 렌더링하여 상태 갱신
    rerender(<TagList initialTags={mockTags.filter(tag => tag.id !== '1')} />);
  ...
  it('API 응답에 error 속성이 없을 때 기본 오류 메시지를 사용해야 함', async () => {
    ...
    (global.fetch as Mock).mockResolvedValue({
      ok: false,
      status: 400,
      json: async () => ({ status: "error" }) // error 속성이 없는 응답
    } as Response);
  ...
  // 브랜치 커버리지 테스트: error가 Error 인스턴스가 아닐 때 (70번 라인)
  it('error가 Error 인스턴스가 아닐 때 기본 오류 메시지를 사용해야 함', async () => {
    // 문자열 에러를 던지도록 모킹
    (global.fetch as Mock).mockImplementation(() => {
      throw "일반 문자열 에러"; // Error 인스턴스가 아닌 값
    });
